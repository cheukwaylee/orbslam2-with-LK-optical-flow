...
cv::Mat Tracking::GrabImageMonocular(const cv::Mat &im, const double &timestamp)
{
    ...
    if (!needNewKF)
    {
        ...
        cv::Mat mTcw;
        last_mnMatchesInliers = mnMatchesInliers;
        LKimg = computeMtcwUseLK(mpLastKeyFrame, im, mCurrentFrame.mnId - mpLastKeyFrame->mnFrameId == 1, mK, mDistCoef, mTcw, mnMatchesInliers);
        //        LKimg = flow(mpLastKeyFrame, imRGB,  mCurrentFrame.mnId - mpLastKeyFrame->mnFrameId ==1, mK, mDistCoef, mTcw);
        mpFrameDrawer->LK = LKimg;
        mCurrentFrame.mTcw = mTcw;
        clock_t b = clock();
...
bool Tracking::NeedNewKeyFrame()
{
    ...
    const bool c3 = mnMatchesInliers < 300 && mnMatchesInliers > 0;
    const bool c4 = mnMatchesInliers < 100 && mnMatchesInliers > 0;
    const bool c5 = last_mnMatchesInliers / (mnMatchesInliers + 0.0001) > 2; ///inlier decrease fast
